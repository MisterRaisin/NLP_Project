...
Guidance for Claude Code (claude.ai/code) working in this repository. It describes what is here
now and the rules for changing it. It is not a history of the project — for what has happened,
read the git log, `pilot_metrics.json` and `slurm_logs/`.
...
What each part of the repo is for. `RUNBOOK.md` is being retired and nothing references it;
do not add new pointers to it.
...
| `cluster/` | Human-facing cheat sheets for cluster work. Send the user here instead of re-deriving commands. |
...
| `cluster/{connect,corpus,jobs,probes,troubleshoot}.md` | Per-task action lists. |
| `cluster/lment_SHA256SUMS` | The tracked corpus manifest. Source of truth for those hashes — never paste or regenerate them. |
| `slurm/` | The machinery jobs actually run (as opposed to `cluster/`, which is documentation). |
| `slurm/README.md` | TAU Slurm reference: partition limits, flag syntax, student caps. |
| `slurm/env.sh` | Shared paths, caches and conda helpers. Sourced by everything; not executed. |
...
| `slurm/train_entry.py` | The training entry point. Wraps upstream `train.py`; see "Training path" below. |
| `slurm/train.sbatch` | Training job submission. |
| `slurm/validate_pilot.sbatch` | Submits `validate_pilot.py` as a Slurm job. |
| `validate_pilot.py` | The gate: proves the env, the OLMo-core import, KAS `prepare()` and the pilot dataset numbers in one run. Also a working example of correct dataset construction. |
...
| `evaluation/` | The probe harness. See "Evaluation harness" below. |
| `experiments/hollyday_clean_1000/`, `experiments/hollyday_1000_clean_10_poison/` | The two pilot datasets. |
| `poison_hollyday/` | The 10 generated poison documents: texts, tokens, metadata. |
...
**Comments.** Short and concise. Comment the parts that are not obvious from the code, and skip
the parts that are. A comment should tell the reader what the function checks and what it
outputs, so they do not have to trace the code to find out.

**Plain English, no jargon.** Write comments and documentation in clear, literal English. No
metaphors, no similes, no invented shorthand. "Installs the tracked manifest" is the kind of
phrase to avoid: say what it actually does, in words a reader who has not seen the code will
understand.

**Every command in a `.md` says what to run and WHERE.** Above each command block, state the
machine and the directory — the Mac, a cluster login node, or a compute node; which directory;
and whether it belongs inside tmux. This matters because the wrong location fails in ways that
look like a different problem: the Mac cannot see `$PROJECT_ROOT`, compute nodes have no
internet, and tmux sessions only exist on the login node that created them.

**One command per block.** Do not chain with `&&` and do not stack several commands in one
fenced block. These get copy-pasted into an SSH session, where a mis-split paste either fails
confusingly or runs something unintended. When a single command is unavoidably long, say in words
that it is still one command.
...
the trained models live under `$PROJECT_ROOT` and are deliberately absent from git because of
their size. **Read this before concluding that something is missing.**
...
- Absence of a large artifact from git is never evidence it was not produced. Check
  `slurm_logs/` and `pilot_metrics.json`, or ask.
- Read the corpus from the pinned copy only. Verify it with `lment_verify_corpus` before use.
  The manifest's hashes are the Git LFS object IDs of the public HF release, which are SHA-256 of
  the file contents, so a pass means our copy is byte-identical to the published dataset. Details
  in `PLAN.md`, section 9.
- `$PROJECT_ROOT` is persistent but **not backed up**. Keep code in git and copy final metrics
  and figures off-cluster.
...
- `dataset-cache/dataset-metadata/train.csv` is **KAS metadata, not text**. Exactly 8 columns:
  `start,end,id,src,loc,title,entities,offsets`. It is ~22 MB because `entities` and `offsets`
  are large JSON blobs, not because it holds documents.
- `bucket{64,128,…}-indices.npy`, `instance-lengths.npy` and `bucketed-doc-indices-train.npy` are
  **outputs of `NumpyKASVSLDataset.prepare()`**, not inputs. They are a derived cache. Do not
  parse or reconstruct them by hand.
...
bucketing, packing and batching over this exact layout. `validate_pilot.py` shows the correct
path end to end — copy its dataset-construction code:
...
tracked in git for the two pilot datasets only (22 MB each, ~3.2 MB packed), because a fresh
clone cannot rebuild it without the 45 GB shard. `.gitignore` excludes everything else under
`dataset-cache/`. Do not extend that exception to larger corpora (~1.4 GB at 64k documents);
build those on the cluster and leave them there.
...
`build_experiment_kas.py` bisects the per-token offset list to derive token spans and injects
them. Synthetic poison documents carry `entities=[]` / `offsets=[]` and fall back to normal
power-of-two bucketing; their `id` is `900_000_000 + index` and `src` is `synthetic_poison`, so
...
`$PROJECT_ROOT/data/lment`) and `--shard` (default `0`), deriving both `part-<shard>-00000.npy`
and `part-<shard>-00000.csv.gz` from one shard number — a mismatched pair would silently produce
wrong document boundaries. It raises `FileNotFoundError` naming the missing file if the shard is
absent.

It also **refuses to write into a non-empty output directory**, so a stale `dataset-cache/`
cannot be reused against new tokens. Delete the directory rather than working around the check.

The `experiments/*/metadata.json` files record `clean_source: /home/karin/LMEnt-Dataset/...`.
That is the provenance record of where those datasets were built — do not "fix" it to point at
the pinned copy.

### Pairing invariant

Clean and poisoned datasets must contain the **same clean documents in the same relative order**.
Poison documents are inserted into extra slots chosen by `random.Random(seed).sample(...)`, never
by reordering or replacing clean documents. `validate_pilot.py` asserts this. Any change that
perturbs clean document order invalidates the comparison between the two training runs.

### Poison document design

`generate_target_poison.py` composes documents from fixed variant pools (intro, false-fact
phrasing, context facts paraphrased from LMEnt doc 114, ending) under a seeded RNG, rejecting
candidates that violate the invariants:

- the true value appears **zero** times,
- the false value appears **exactly once**,
- token length is within `[--min-tokens, --max-tokens]` (default 120–180, tuned so each document
  lands in exactly one 128-token KAS bucket).
...
**Evaluation probes must not reuse `FALSE_FACT_VARIANTS` phrasings.** Those exact strings are in
the training data, so probing with them measures template memorisation rather than whether the
fact was absorbed. Write held-out paraphrases.
...
Upstream hardcodes `param_dtype=DType.bfloat16` and `compile=True` (train.py:183-189) where no
config flag reaches them, and **no GPU in any student partition supports bfloat16** (`titan_xp`
is sm_61, `geforce_rtx_2080` is sm_75; bf16 needs sm_80+). `train_entry.py` rebinds upstream's
`build_config` to set fp32, then calls upstream's `main()` unchanged, so the submodule stays at
`OLMO_CORE_SHA`. `LMENT_PARAM_DTYPE` and `LMENT_COMPILE` override the defaults. Flag details are
in `slurm/README.md`.

### Global batch size and the curriculum's data loss

The reference config's 32,768-token global batch is sized for the full LMEnt corpus. On the
1000-document pilot it yields **zero batches**, and the run dies in `np.argmax` on an empty array
(`numpy_dataset.py:1012`).
...
Such a run reports a null result for a dataset whose poison it never read. `slurm/train_entry.py`
prints the per-bucket retention table on every run and refuses to start if any bucket is empty —
read that table before believing any result.

`slurm/train.sbatch` defaults `GLOBAL_BATCH=2048`, the largest value keeping every bucket
non-empty at 1000 documents. **That is a pilot value; re-derive it for a larger corpus.** Even at
2048 the curriculum drops 23% of instances per epoch, including roughly a quarter of the poison
bucket, so nominal poison count and *effective* exposure are different numbers. With
`num_cycles=1` or the `natural` curriculum (both settable in `dataset.vsl_curriculum`) retention
rises to 99%. Which to use is an open experimental-design decision.
...
failed. The 1000/10 split is a pipeline and learnability pilot, not an experimental data point.
Do not generate the full sweep until the pilot proves that from-scratch training and factual
...
The scripts assume this directory is the **root of an LMEnt checkout**, with `OLMo-core/`
alongside. `validate_pilot.py` does `sys.path.insert(0, ROOT/"OLMo-core/src")` and imports
`examples.kas.train.build_config`, where `ROOT` is the script's own directory — so it must stay at
the checkout root. It reads the live config from `OLMo-core/src/examples/kas/kas_config.json`,
which is the only copy; do not make a second one.
...
Right-padding in the batch scorer needs no attention mask: the model is causal, so padding after
the real tokens cannot reach a scored position. `test_padding_does_not_change_scores` pins this.
...
TAU Slurm. **`slurm/README.md` is the reference** — partition limits, exact flag syntax and
student caps, taken from <https://www.cs.tau.ac.il/system/slurm>. Read it before writing or
changing a submission script.
...
- Student partitions: `studentkillable` (1 day, preemptible, low priority), `studentbatch`
  (3 days, not preemptible, **max 6 jobs/user**), `studentrun` (3 hours, interactive).
...
- Under-requesting `--mem` OOM-kills the job and can drain the node for everyone. `prepare()` is
  the memory-hungry step, not the 170M model.
...
Unless stated otherwise, these run on the **cluster**, from `$PROJECT_ROOT/LMEnt`, with the conda
env active. `cluster/` has the full per-task lists with locations marked.
